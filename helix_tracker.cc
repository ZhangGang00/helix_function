...
int main(int argc, char ** argv) {
	...
	for (const FaserTrackerTruthParticle * tp : event->truthParticles) 
	  {
	    //if (std::abs(tp->pdgCode)==13 && tp->parentID==1)
	    if (tp->pdgCode==13)
	     {
	       pdgID = tp->pdgCode;
	       find_truth=1;
	...
	N_hit_plane=0;
	for (int i=0; i<8; i++) { for (int j=0; j<10; j++) hit[i][j]=0; }
	for (int i=0; i<8; i++) { for (int j=0; j<5; j++) hit_rowID[i][j]=0; }
        ...
        for (const FaserTrackerSpacePoint * sp : event->spacePoints) 
	{
	    ...
	    space_points_x.push_back(pos.X());
	    space_points_y.push_back(pos.Y());
	    space_points_z.push_back(pos.Z());
	    for (int i=0; i<8; i++) { 
	       if (std::fabs(pos.Z()-target_z[i]) < 1) 
	        {
		   hit[i][2*(int)hit[i][0]+1]=pos.X();
		   hit[i][2*(int)hit[i][0]+2]=pos.Y();
	...
	while (hit[k][0] != 1) { k++; }  // k_th plane is the first plane with only one space point
        for (int i=0; i<8; i++)  // keep the space point closest to h[k][] in each plane
	  {
	     if (hit[i][0] > 0) {
